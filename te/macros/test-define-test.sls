...
(define-test-case (test-$define-test:data-thunks)

  (define-test ("data thunk transfer")
    (define test
      ($ ($define-test '() '()
           '((define-test ("Sum" a b) (+ a b)) ((list (list 1 2)))) )) )
    (equal? '((1 2))
      ((test-data test)) ) )
...
(verify-test-case! test-$define-test:data-thunks)
